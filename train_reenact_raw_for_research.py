...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path to the reference motion video file (e.g., ./assets/jumping_jacks.mp4). "
    ...
    parser.add_argument(
        "--same_noise_over_iterations",
        action="store_true",
        help="Set it on to set the same seed on each iteration, resulting in the same noise latents inserted",
    )
    parser.add_argument(
        "--different_noise_over_iterations_cond",
        action="store_true",
        help="should only matter if same_noise_over_iterations is turned on",
    )
    parser.add_argument(
        "--different_noise_over_iterations_latents",
        action="store_true",
        help="should only matter if same_noise_over_iterations is turned on",
    )
    parser.add_argument(
        "--different_noise_over_iterations_time_ids",
        action="store_true",
        help="should only matter if same_noise_over_iterations is turned on",
    )
    parser.add_argument(
...
def main():
    ...
    if args.resume_from_checkpoint_dir is None:
        video_name = os.path.basename(args.video_to_inverse)
        args.output_dir = add_timestamp_suff(
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            ...
            for i, model in enumerate(models):
                ...
                elif isinstance(model, UNetSpatioTemporalConditionModel):
    ...
    val_dataset = SimpleImagesDataset(
        args.validation_images_path,
        width=args.width,
        height=args.height,
        max_images_n=args.num_validation_images,
        device=accelerator.device,
    )
    train_dataset = MotionSingleVideoDataset(
        args.video_to_inverse,
        size=(args.num_frames, args.width, args.height),
        device=accelerator.device,
    )
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            image_embds_wrap.load_tensor(
                os.path.join(args.output_dir, path)
            )  # TODO: handle optimizer saving and resuming
    ...
    # Prepare everything with our `accelerator`. #CHANGED HERE - added image_embds_wrap
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(image_embds_wrap):
                # Sample with different noise in this section
                # Note - didn't check for the vae un-determenistic nature and didn't try to encode using the vae without a const seed. Mightbe important but I guess not.
                # step > 0 to not case about stuff like that we need to first intialize the latents to know the shape of the noise (e.g. bsz)
                if args.different_noise_over_iterations_latents and step > 0:
                ...
                if args.different_noise_over_iterations_cond and step > 0:
                    # Note that noise_aug_strength "determenistic or not" comes from the cond_sigmas "determenistic or not". Didn't distentangle them since I didn't want to get into the meaning of this move
                ...
                if args.different_noise_over_iterations_time_ids and step > 0:
                    # Note that noise_aug_strength "determenistic or not" comes from the cond_sigmas "determenistic or not". Didn't distentangle them since I didn't want to get into the meaning of this move
                ...
                # From here on we sample with the same exact noise in each section:
                if args.same_noise_over_iterations:
                    torch.manual_seed(0)
                ...
                if not args.different_noise_over_iterations_cond or step == 0:
                    cond_noise = torch.randn_like(conditional_pixel_values)
                    cond_sigmas = rand_log_normal(
                        shape=[
                            bsz,
                ...
                conditional_pixel_values = (
                    cond_noise * cond_sigmas + conditional_pixel_values
                )
                ...
                if not args.different_noise_over_iterations_latents or step == 0:
                    noise = torch.randn_like(latents)
                    sigmas = rand_log_normal(
                        shape=[
                            bsz,
                ...
                if not args.different_noise_over_iterations_time_ids or step == 0:
                    added_time_ids = _get_add_time_ids(
                        7,  # fixed
                        127,  # motion_bucket_id = 127, fixed
                        noise_aug_strength,  # noise_aug_strength == cond_sigmas
                ...
                assert (
                    args.conditioning_dropout_prob is None
                )  # CHANGED HERE - to make sure I don't use it
                ...
                target = latents
                model_pred = (
                    unet(  # CHANGED HERE "encoder_hidden_states" to image_embds_wrap()
                        inp_noisy_latents,
                        timesteps,
                        image_embds_wrap(),
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        image_embds_wrap.save_pretrained(save_path)
                    ...
                    if (global_step % args.validation_steps == 0) or (global_step == 1):
                        ...
                        print(
                            "Offloading training models from GPU to CPU during validation"
                        )
                        ...
                        with torch.autocast(
                            ...
                            with torch.no_grad():
                                ...
                                training_decoded = pipeline_decode_latents(
                                    ...
                                    args.num_frames,
                                    decode_chunk_size=args.num_frames // 2 + 1,
                                )[0]
                                log_decoded_video(
                                    global_step,
                                    args.num_frames,
                        ...
                        # Reload frmo cpu to GPU after validation
    ...
    if accelerator.is_main_process:
        ...
        image_embds_wrap.save_pretrained(args.output_dir)
...
if __name__ == "__main__":
    # NOTE: This file is intended for flexible, raw research purposes.
    # It is not well organized or production-ready—structure, style, and argument parsing are ad-hoc for fast iteration.

    import sys

    vid_path = "./assets/jumping_jacks.mp4"
    val_dir = "./images_for_validation/gs_person"
    resume_from = None  # "./outputs/jumping_jacks_2_2024-09-29_00-35-49"

    # fmt: off
    # Add those only of not provided
    if not "--video_to_inverse" in sys.argv and not "--validation_images_path" in sys.argv:
        sys.argv += [
            "--video_to_inverse", vid_path,
            "--validation_images_path", val_dir
        ]
    # always add those:
    sys.argv += [
            "--pretrained_model_name_or_path", "stabilityai/stable-video-diffusion-img2vid",
            # As long as we don't use augmentations, increasing it above 1 will be stupid and result in lower effective iterations number.
            # UNLESS there is noise in the training process. Which I think we do have so it's kind of like augmentations in that manner
            "--gradient_accumulation_steps", "1", 
            "--seed", "0",

            "--num_frames", "14",
            # "--width", "512", # ERASE IT, JUST FOR DEBUGGING
            # "--height", "512", # ERASE IT, JUST FOR DEBUGGING

            "--gradient_checkpointing", # Neccessary to fit in the gpu even with 14 frames and non-xt model.
            "--enable_xformers_memory_efficient_attention", # Neccessary, from 26s/it got me to 7.5s/it

            "--num_workers", "0", # to get the data to be loaded in the main process, otherwise i had errors and I don't acre about it
            "--max_train_steps", "5000",
            "--learning_rate", "0.01", # default is 0.0001
            # "--conditioning_dropout_prob", "0.1", # To avoid it

            "--num_tokens_in_motion_features", "5",

            # "--same_noise_over_iterations", # To debug and assert that with static noise over iterations, the loss is stable and decreasing
            # "--different_noise_over_iterations_cond",
            # "--different_noise_over_iterations_latents",
            # "--different_noise_over_iterations_time_ids",

            "--mixed_precision", "fp16", #7.5s/it -> 2.19s/it; 54GB -> 42GB

            # Added for intermediate logging
    ...
    if resume_from is not None:
        sys.argv += [
            "--resume_from_checkpoint_dir", resume_from,
